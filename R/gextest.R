...
gextest<-function(genome.test, nhazard, results.graph, lim_chi, global_test_choice, pcorrd, pcorrv)
{
  ...
  p = as.numeric(wilcox.test(as.matrix(as.numeric(gentest[,"list"])), as.matrix(as.numeric(gentest[,"hazard"])), paired = TRUE, exact = FALSE, correct = FALSE)[3])
  if(p<=0.05)
  {
  ...
  for(N in 1:max)
  {
    ...
    if(N>=2)   
    {
      if(floor(nrow(gentest)/N)*N==nrow(gentest))
      {
        ...
        #write(paste("Adjusting concatenation from ", (floor(nrow(gentest)/N)*N), " to ", nrow(gentest), sep=""), file="")
        x = matrix(0, ncol=1, nrow=(floor(nrow(gentest)/N)))
        theo = matrix(0, ncol=1, nrow=(floor(nrow(gentest)/N)))
        theo_tmp = as.matrix(gentest[,"theo"])
    ...
    #test
    if(is.nan(as.numeric(chisq.test(x, p=theo)[3]))|as.numeric(chisq.test(x, p=theo)[3])>0.05)
    {
      g_test[I,N] = ""
      g_test_graph[I,N] = ""
    }else{
      g_test_graph[I,N] = as.numeric(chisq.test(x, p=theo)[3])
      g_test[I,N] = paste(as.numeric(chisq.test(x, p=theo)[3]), "(", min(x), ")", sep="")
      #test of the minimal number of genes by unit
      if((min(x)>=lim_chi)&(global_tests[global_tests[,1]==I,2]==0)) #ok
      {
  ...
  #write(paste("Local test", sep=""), file="")
  ...
  if(pass_to_local)
  {
    ...
    for(U in 1:(nrow(gentest)))
    {
      ...
      {
        regions[U,1]=1
        test[U,] = as.numeric(binom.test(as.numeric(gentest[U,"list"]), totlist, as.numeric(gentest[U,"total"]) / nhazard)[3])
      } else{
      regions[U,1]=0
      test[U,] = 1
    }
  ...
  for(C in levels(as.data.frame(genome.test.tmp)[,"chr"]))
  {
    ...
    pval[,1] = gexChr[, "unit"]
        ...
        {
          #write(paste("Test region ", I, sep=""), file="")
          ...
          p = as.numeric(p[3])
          #write(paste("Entire region p=", p, sep=""), file="")
          #saving results
  ...
  #write.table(local_test_results, row.names=TRUE, sep="\t", file = file.path(paste(results.graph, "local_test_results.txt", sep="")));
